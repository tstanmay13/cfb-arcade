...
| 94 | 26% → **38%** | 4% → 3% |
| 97 target | 40% → **62%** | 2% → 1% |
| Tier0 row (≥97) | 45% → **70%** | 2% → 0% |

Conditional odds a player now sees: **91 → 17% natty** (miss-the-CFP 7.5%),
**93 → 31%**, **95 → 46%** (QF exit down to 20%), **≥97 → 70%** — commanding,
never guaranteed (the 0026 reversal of the auto-natty stands; dynasty still
rides an 80% roll on a Tier0 natty). Tier1's informational row re-pins to
`outcomeOdds(91)` per the 0026 convention.

## Measured (20k drafts/policy, scripts/balance.ts, 2026-07-16 bake)

| metric | random | skilled | oracle |
|---|---|---|---|
| 16-0 rate (0032 → 0033) | 3.10% → 3.10% | 7.5% → **8.74%** | 17.3% → **24.01%** |
...
- **Ladder** (ADR-0029 ratio contract): skilled/random **2.82×**,
  oracle/skilled **2.75×** — both above the ≥2× / ≥2.2× gates. Oracle's
  24% absolute rate is a deliberate consequence of the favorite law: a
...
- *Conditional* elite odds — floor: a 95-power board is a favorite to reach
  the final and ~coin-flip-plus to win it; ≥97 is ~70% (move the 94/97
  anchors and Tier0 row).
